# Dars 25: mahsulot yaratish uchun ADMIN yoki MANAGER kerak.
# Shuning uchun conftest dagi rol-based fixture'lardan foydalanamiz.
...
def test_create_product(client, admin_user):
    """ADMIN mahsulot yarata oladi (Dars 25: require_role)"""
...
def test_update_product(client, admin_user):
    headers = make_auth_headers(admin_user)
    category_response = client.post("/categories/", json={"name": "Elektronika"})
    category_id = category_response.json()["id"]
...
def test_delete_product(client, admin_user):
    headers = make_auth_headers(admin_user)
    category_response = client.post("/categories/", json={"name": "Elektronika"})
    category_id = category_response.json()["id"]
    ...
    # O'chirish uchun ham ADMIN kerak (require_role(UserRole.ADMIN))
...
def test_products_filtering(client, admin_user):
    headers = make_auth_headers(admin_user)
    category_response = client.post("/categories/", json={"name": "Elektronika"})
    category_id = category_response.json()["id"]
...
def test_products_pagination(client, admin_user):
    headers = make_auth_headers(admin_user)
    category_response = client.post("/categories/", json={"name": "Elektronika"})
    category_id = category_response.json()["id"]
